...
.PHONY: test-int test-int-up test-int-down test-int-clean test-int-build java-test test-e2e test-mcp run-demo-mcp
...
# Unit tests + demo-data MCP tool-call smoke test against a fresh demo stack.
# Caveats: the smoke test's exactly-+10 summary delta assumes the run does not cross
# midnight in the app's timezone between checks; the stack cannot run alongside
# run-demo/test-e2e (fixed container names). Requires a committed package-lock.json (npm ci).
test-mcp: build
