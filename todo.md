...
- [ ] **Transaction update from UI is broken** — `TransactionController.updateTransaction` takes raw `Transaction` JPA entity as `@RequestBody` instead of a DTO. This causes: (1) Labels — frontend sends `labels: [{id}]` but backend expects `Set<Label>`, Jackson deserializes into partial `Label` objects with `name=null`; (2) Account change — frontend sends `accountId` string, entity has no `accountId` field (only `Account account`), Jackson ignores it; (3) Category change — same pattern, `categoryId` ignored; (4) `labelIds` key is sent as `undefined` and dropped by JSON serialization. Contrast: `TransferController` uses `TransferRequest` DTO with explicit `labelIds: List<UUID>` and `categoryId: UUID` — `TransactionController` should follow the same pattern. (5) No unit test for update in `TransactionControllerTest`. (6) No integration test for update in `TransactionIntegrationTest`.
- [ ] **Transfer update from UI also wipes category/labels** — `TransferService.updateTransfer` always overwrites category and labels, even if the user only edited the date. The frontend sends `categoryId: null` when the category field is empty, and the service sets `existing.setCategory(null)` — same for labels (clears to empty set). Any edit without explicit category/label selection wipes them.
